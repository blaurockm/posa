...
public class KleinteilKopfMapper implements ResultSetMapper<KleinteilKopf> {
	...
	public KleinteilKopf map(int arg0, ResultSet rs, StatementContext ctx) throws SQLException {
		...
		vorg.setRechnungsNummer(rs.getInt("RechnungNummer"));
		...
		vorg.setNetto0(rs.getBigDecimal("RechnungBetragMwst_0"));
		vorg.setNetto7(rs.getBigDecimal("RechnungBetragMwst_7"));
		vorg.setNetto19(rs.getBigDecimal("RechnungBetragMwst_16"));
		vorg.setErfassungsDatum(new DateTime(rs.getTimestamp("ErfassungsDatum")));
		vorg.setRechnungsDatum(new DateTime(rs.getTimestamp("RechnungsDatum")));
